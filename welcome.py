...
TEMP_FILENAME = "printNameBadge.txt"

def printNameBadge(name): #prints name badge from user's inputted information
    p = open(TEMP_FILENAME, "w")
    p.write("Hello My Name Is:")
    p.write("\n")
    p.write("\n")
    p.write(name)
    p.close()
    os.system("lpr -o landscape -o lpi=2 -o cpi=5 -o PageSize={} {}".format(PAGE_SIZE, TEMP_FILENAME))

def printQRCode(name, email):
    """
    prints QR code from user's inputted information
    """
    os.system("qrencode -o {}.png " + "'{};{}'".format(email, name, email))
    os.system("lpr -o landscape -o PageSize={} {}.png".format(PAGE_SIZE, email))

def surveyQuestions(name):
    """
    survey questions segment, can be expanded to more than one question
    """
...
def userInput():
    """
    This is the new user function
    """
    userName = ""
    userEmail = ""
    correctName = "no"
    while correctName == "no" or correctName == "n":
...
def thankYouMessage(name):
    """
    takes name parameter and puts it in a thank you message
    """
    os.system("clear")
...
def qrTurnOffProcess(barcodedata):
    """
    turns off camera
    """
    if barcodedata:
